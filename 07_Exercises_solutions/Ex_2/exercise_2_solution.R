...
#----- Create a map showing the spatial location of Mauch Chunk Lake

# We are only plotting the location of a single lake (one x,y coordiate)
# so we can just grab the first row of dat_tot_catch for our lat/long data

# Subset first row and all columns of dat_tot_catch
map_dat <- dat_tot_catch[1,]
# Convert map_dat to a spatial object (simple feature)
# and set its coordinate references system (crs)
map_dat <- st_as_sf(map_dat, coords = c("long", "lat"), crs = 4326)

# Grab state boundaries from the spData package and
...
# Plot catch-per-unit-effort data over time for select species

# Subset table1 for the species Largemouth Bass, Smallmouth Bass, 
# and Walleye. We need to ungroup table1 first, then filter and mutate,
# and use complete to fill in years
